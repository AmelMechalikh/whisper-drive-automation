...
Worker Cloud Run pour générer les sous-titres brûlés en utilisant RunPod GPU backend
...
current_dir = Path(__file__).parent.parent  # Remonter au root du projet
...
def main():
    """Boucle principale du worker sous-titres RunPod"""
    logger.info("=" * 60)
    logger.info("📺 RunPod Transcription Worker démarré")
    logger.info("=" * 60)
    ...
    try:
        ...
        # Use service account when running in Cloud Run (no credentials file)
        ...
        if credentials_path.exists():
            ...
            logger.info("Using service account (Cloud Run)")
            drive_manager = DriveManager(credentials_path=None)
        ...
        while True:
            try:
                ...
                if not job_files:
                    logger.info(f"📭 Aucun job en attente")
                else:
    ...
    logger.info("👋 RunPod Transcription Worker arrêté")
    return 0
