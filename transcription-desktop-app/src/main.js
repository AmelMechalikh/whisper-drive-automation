...
} catch { /* ffmpeg-static not installed yet — use system ffmpeg */ }

const automationWorker = require('./services/automation-worker');

// Store pour sauvegarder les credentials Google
const store = new Store();

// EventEmitter to bridge automation-worker → renderer
const automationEvents = new EventEmitter();

// Automation state
let automationEnabled = false;
let machineId = store.get('machine_id');
if (!machineId) {
  machineId = crypto.randomUUID();
  store.set('machine_id', machineId);
}
...
const { CLIENT_ID, CLIENT_SECRET } = require('./oauth-credentials');
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    },
    {
      ...
      click: () => {
        ...
        if (mainWindow) mainWindow.webContents.send('automation-status', getAutomationStatus());
      }
    },
    {
      label: 'Watch Folder',
  ...
  return new Promise((resolve, reject) => {
    ...
    server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        // Port already in use — tokens were likely already saved, just resolve
        server.close();
        resolve({ success: true });
      } else {
        reject(err);
...
function startWatchingFolder(folderPath) {
  ...
  // Safe console.log - supprimé pour éviter EPIPE
        // console.log(`👁️ Surveillance activée: ${folderPath}`);
  return true;
}
...
async function processVideoAuto(videoPath) {
  ...
  try {
    ...
    if (audioExists && videoExists) {
      // Les deux fichiers existent déjà, skip complètement
      if (mainWindow) {
        mainWindow.webContents.send('watch-folder-progress', {
          fileName: fileName,
          stage: 'skipped',
    ...
    if (!audioExists) {
      ...
      audioResult = await uploadToDrive(
        ...
        fileName,  // watchFileName pour les notifications
        'upload-audio'  // stage
      );

      if (!audioResult.success) {
        // Nettoyer fichier audio temporaire
        try {
    ...
    // Étape 2: Upload vidéo (si pas déjà sur Drive)
    ...
    if (!videoExists) {
      ...
      if (!videoResult.success) {
        // Nettoyer fichier audio temporaire si créé
        if (audioPath) {
          try {
            fs.unlinkSync(audioPath);
          } catch (e) {}
...
ipcMain.handle('process-video', async (event, videoPath) => {
  try {
    ...
    // 2. Upload audio vers Drive (PRIORITAIRE pour démarrer la transcription)
    mainWindow.webContents.send('status-update', {
      stage: 'upload-audio',
    ...
    // Vérifier si l'upload audio a réussi
    if (!audioResult.success) {
      fs.unlinkSync(audioPath);
      return {
        success: false,
    ...
    // 3. Upload vidéo vers Drive (après l'audio)
    mainWindow.webContents.send('status-update', {
      stage: 'upload-video',
    ...
    // Vérifier si l'upload vidéo a réussi
    if (!videoResult.success) {
      fs.unlinkSync(audioPath);
      return {
        success: false,
    ...
    // 4. Nettoyer le fichier audio local
    fs.unlinkSync(audioPath);
...
function startAutomation() {
  ...
  if (!oauth2Client || !oauth2Client.credentials || !oauth2Client.credentials.access_token) {
    if (mainWindow) mainWindow.webContents.send('log-line', { message: '❌ Non authentifié — connectez-vous avec Google', time: new Date().toISOString() });
    return;
  }
  automationEnabled = true;

  // Wire up events → renderer
  automationEvents.on('log', (data) => {
    if (mainWindow) mainWindow.webContents.send('log-line', data);
  });
  automationEvents.on('status-update', (data) => {
    if (mainWindow) mainWindow.webContents.send('automation-status', data);
  });
  automationEvents.on('transcription-complete', (data) => {
    if (mainWindow) mainWindow.webContents.send('automation-update', { type: 'transcription-complete', ...data });
  });
  automationEvents.on('highlights-complete', (data) => {
    if (mainWindow) mainWindow.webContents.send('automation-update', { type: 'highlights-complete', ...data });
  });
  automationEvents.on('auth-error', () => {
    if (mainWindow) mainWindow.webContents.send('automation-update', { type: 'auth-error' });
    stopAutomation();
  });
...
ipcMain.handle('run-transcription-now', async () => {
  automationWorker.runTranscriptionNow();
  return { success: true };
});

ipcMain.handle('run-highlights-now', async () => {
  automationWorker.runHighlightsNow();
  return { success: true };
});
...
ipcMain.handle('save-settings', async (event, newSettings) => {
  ...
  // Restart automation with new settings if running
  if (automationEnabled) {
    stopAutomation();
    startAutomation();
  }
...
ipcMain.handle('test-r2-connection', async (event, r2Config) => {
  try {
    const R2Uploader = require('./services/r2-uploader');
    const uploader = new R2Uploader(r2Config);
    const url = await uploader.testConnection();
...
ipcMain.handle('test-runpod-connection', async (event, runpodConfig) => {
  try {
    const RunPodClient = require('./services/runpod-client');
    const endpoint = `https://api.runpod.ai/v2/${runpodConfig.endpointId}`;
    const client = new RunPodClient(runpodConfig.apiKey, endpoint);
    // Just verify the endpoint is reachable by checking a fake job status
    // (will 404 but confirms credentials/connectivity)
    await client._request('GET', `${endpoint}/status/health-check`).catch(() => {});
    return { success: true };
  } catch (err) {
    return { success: false, error: err.message };
  }
      ...
      try {
        ...
        if (isAuth && oauth2Client.credentials && oauth2Client.credentials.access_token) {
          ...
          if (mainWindow && mainWindow.webContents) {
            mainWindow.webContents.send('watch-folder-auto-started', { folderPath: savedFolder });
          }
        }
